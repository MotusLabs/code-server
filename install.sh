...
echo_latest_version() {
  if [ "${EDGE-}" ]; then
    version="$(curl -fsSL "https://api.github.com/repos/$REPO/releases" | awk 'match($0,/.*"html_url": "(.*\/releases\/tag\/.*)".*/)' | head -n 1 | awk -F '"' '{print $4}')"
  else
...
# The repository the release assets are pulled from. This fork carries patches
# that upstream does not have, so the default has to be the fork.
REPO=${REPO:-MotusLabs/code-server}
...
install_deb() {
  ...
  fetch "https://github.com/$REPO/releases/download/v$VERSION/code-server_${VERSION}_$ARCH.deb" \
...
install_rpm() {
  ...
  fetch "https://github.com/$REPO/releases/download/v$VERSION/code-server-$VERSION-$ARCH.rpm" \
...
install_standalone() {
  ...
  fetch "https://github.com/$REPO/releases/download/v$VERSION/code-server-$VERSION-$OS-$ARCH.tar.gz" \
